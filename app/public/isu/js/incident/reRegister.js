...
$(document).ready(function () {
    //접수내용
    $('#_status_nm').html(incidentObj.status_nm);
    if(incidentObj.process_speed == "Y"){
        $('#_process_speed').html('<span class="label label-warning">✔</span>');
    }
    $('#_higher_nm').html(incidentObj.higher_nm);
    $('#_lower_nm').html(incidentObj.lower_nm);
    $('#_request_company_nm-request_nm').html(incidentObj.request_company_nm+"/"+incidentObj.request_nm);
    $('#_request_complete_date').html(incidentObj.request_complete_date);
    $('#_app_menu').html(incidentObj.app_menu);
    $('#_register_nm-register_date').html(incidentObj.register_nm+"/"+incidentObj.register_date);
    $('#_title').html(incidentObj.title);
    $('#_content').html(incidentObj.content);
    $('#_attach').html(incidentObj.attach);


    //처리내용
    $('#_manager_nm').html(incidentObj.manager_nm);
    $('#_receipt_date').html(incidentObj.receipt_date);
    $('#_complete_reserve_date').html(incidentObj.complete_reserve_date);
    $('#_business_level').html(incidentObj.business_level);
    $('#_process_nm').html(incidentObj.process_nm);
    $('#_complete_content').html(incidentObj.complete_content.replace(/\r\n/gi,"<br/>"));
    $('#_complete_date').html(incidentObj.complete_date);
    $('#_work_time').html(incidentObj.work_time);
    $('#_delay_reason').html(incidentObj.delay_reason);
    $('#_valuation').html(incidentObj.valuation);
    $('#_complete_open_flag').html(incidentObj.complete_open_flag);


    //$('#_status_nm').val(incidentObj.status_nm);
    /*
    //엔터키 이벤트 시
